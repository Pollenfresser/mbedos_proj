...
int RS232_OpenComport(gpointer data)
{
  int baudr,
      status;
  char mode[4];
  ...
  mode[0] = wid->device_databits;
  if (wid->status_parity == 0)
    mode[1] = 'N';
  else if (wid->status_parity == 1)
    mode[1] = 'E';
  else if (wid->status_parity == 2)
    mode[1] = 'O';
  mode[2] = wid->device_stopbits;
  mode[3] = 0;
...
*/
  ...
  Cport[wid->idd] = open(dev[wid->idd], O_RDWR | O_NOCTTY | O_NDELAY);
  if(Cport[wid->idd]==-1)
  {
  ...
  if(flock(Cport[wid->idd], LOCK_EX | LOCK_NB) != 0)
  {
    close(Cport[wid->idd]);
  ...
  error = tcgetattr(Cport[wid->idd], old_port_settings + wid->idd);
  if(error==-1)
  {
    close(Cport[wid->idd]);
    flock(Cport[wid->idd], LOCK_UN);  /* free the port so that others can use it. */
  ...
  error = tcsetattr(Cport[wid->idd], TCSANOW, &new_port_settings);
  if(error==-1)
  {
    tcsetattr(Cport[wid->idd], TCSANOW, old_port_settings + wid->idd);
    close(Cport[wid->idd]);
    flock(Cport[wid->idd], LOCK_UN);  /* free the port so that others can use it. */
...
/* http://man7.org/linux/man-pages/man4/tty_ioctl.4.html */
  ...
  if(ioctl(Cport[wid->idd], TIOCMGET, &status) == -1)
  {
    tcsetattr(Cport[wid->idd], TCSANOW, old_port_settings + wid->idd);
    flock(Cport[wid->idd], LOCK_UN);  /* free the port so that others can use it. */
  ...
  if(ioctl(Cport[wid->idd], TIOCMSET, &status) == -1)
  {
    tcsetattr(Cport[wid->idd], TCSANOW, old_port_settings + wid->idd);
    flock(Cport[wid->idd], LOCK_UN);  /* free the port so that others can use it. */
